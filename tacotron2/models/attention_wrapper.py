...
def _location_sensitive_score(W_query, attention_weights):
  ...
  Returns:
	...
	attention_weights = tf.expand_dims(attention_weights, axis=2) # [N, max_time, 1]
  ...
  577–585.
	...
	def __call__(self, query, previous_alignments):
		...
		with variable_scope.variable_scope(None, "location_based_attention", [query]):
			...
			energy = _location_sensitive_score(processed_query, previous_alignments)
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
	...
	def call(self, inputs, state):
		...
		(cell_output, LSTM_output), next_cell_state = self._cell(inputs, state)
		# (concatted output, LSTM output), cell_state:tuple // state:attention_state
		cell_batch_size = (
		...
		for i, attention_mechanism in enumerate(self._attention_mechanisms):
			attention, alignments = _compute_attention(
					attention_mechanism, cell_output, previous_alignments[i], # 여기서 왜 셀아웃풋을 썻는가?
