  ...
  def step(self, time, inputs, state, error, name=None):
    ...
    with ops.name_scope(name, "BasicDecoderStep", (time, inputs, state, error)):
      cell_outputs, cell_state, LSTM_output = self._cell(inputs, state)
        # projection된 cell output, attention state, lstm output
    ...
    return (outputs, next_state, next_inputs, finished, error) # (pojected output, attention_state, next_frame, finished, error)