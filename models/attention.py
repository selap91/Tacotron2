# Modify content-based additive attention "BahdanauAttention"
# Original, αi = Attend(si−1, h)
# Extends, αi = Attend(si−1, αi−1, h)
...
# add alignment to args
def _bahdanau_score(processed_query, keys, alignment, normalize):
  ...
  processed_alignment = array_ops.expand_dims(alignment, 2) # ☆[N, alignments_size, 1]
  processed_alignment = tf.layers.conv1d(processed_alignment, 32, 31, padding="SAME")  # [N, alignments_size, 32]
  processed_alignment = tf.layers.dense(processed_alignment, 128) # [N, alignments_size, 128]
  ...
  if normalize:
    ...
    return math_ops.reduce_sum(
        normed_v * math_ops.tanh(keys + processed_query + alignment + b), [2]) # ☆
  else:
    return math_ops.reduce_sum(v * math_ops.tanh(keys + processed_query + alignment), [2]) # ☆
...
class BahdanauAttention(_BaseAttentionMechanism):
  ...
  def __call__(self, query, state):
    ...
    with variable_scope.variable_scope(None, "bahdanau_attention", [query]):
      ...
      score = _bahdanau_score(processed_query, self._keys, state, self._normalize)
