...
class TacoTestHelper(Helper):
  def __init__(self, batch_size, output_dim, stop_token, r):
    with tf.name_scope('TacoTestHelper'):
      self._batch_size = batch_size
      self._output_dim = output_dim
      self._end_token = tf.tile([0.0], [output_dim * r])
      self._stop_token = stop_token # [N, 1]
  ...
  def next_inputs(self, time, outputs, state, sample_ids, name=None):
    ...
    with tf.name_scope('TacoTestHelper'):
      ...
      finished = tf.reduce_all(self._stop_token, axis=1) # use 'stop-token'
...
class TacoTrainingHelper(Helper):
  ...
  def next_inputs(self, time, outputs, state, sample_ids, name=None):
    with tf.name_scope(name or 'TacoTrainingHelper'):
      ...
      return (finished, next_inputs, state) # (true or false list, time-th frame input, cell_state)
